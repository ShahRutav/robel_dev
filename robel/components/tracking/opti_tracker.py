...
"""Implementation of TrackerComponent using optiTrack to track devices."""
...
class OTTrackerGroupConfig(TrackerGroupConfig):
    """Stores group configuration for a OTTrackerComponent."""
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """Initializes a new configuration for a OTTrackerComponent group.
...
class OTTrackerComponent(TrackerComponent):
    """Component for reading tracking data from a HTC Vive."""

    # Cached OptiTrack client that is shared for the application lifetime.
    _OT_CLIENT = None

    def __init__(self, *args, **kwargs):
        """Initializes a ViveTrackerComponent."""
        super().__init__(*args, **kwargs)
        if self._OT_CLIENT is None:
            from robel.components.tracking.opti_track.client import (
                OTClient)
            self._OT_CLIENT = OTClient()

        # Check that all devices exist.
        for name, group in self.groups.items():
            if group.device_identifier is None:
                continue
            device = self._OT_CLIENT.get_device(group.device_identifier)
    ...
    def _process_group(self, **config_kwargs):
        ...
        return OTTrackerGroupConfig(self.sim_scene, **config_kwargs)

    def _get_group_states(
            self,
            configs: Sequence[OTTrackerGroupConfig],
        ...
        """
        pose_batch = self._OT_CLIENT.get_poses()
        ...
        for config in configs:
            if config.device_identifier is None:
                ...
                continue
            device = self._OT_CLIENT.get_device(config.device_identifier)
    ...
    def set_state(self, state_groups: Dict[str, TrackerState]):
        ...
        self._OT_CLIENT.update_coordinate_system(
