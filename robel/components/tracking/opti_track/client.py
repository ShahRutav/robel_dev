...
import numpy as np
import openvr
import robel.components.tracking.opti_track.optitrack_client as ot

from robel.components.tracking.opti_track.device import VrDevice
from robel.components.tracking.opti_track.poses import (
...
class OTClient:
    ...
    def __init__(self):
        ...
        # Attempt to start optiTrack.
        self._ot_system = ot.opti_client("hulk.cs.washington.edu", 5000, 36)
        self._ot_system.connect()

    def close(self):
        """Cleans up any resources used by the client."""
        if self._ot_system is not None:
            self._ot_system.close()
            self._ot_system = None
    ...
    def discover_devices(self) -> List[VrDevice]:
        ...
        for device_index in range(openvr.k_unMaxTrackedDeviceCount):
            device = VrDevice(self._ot_system, device_index)
    ...
    def opti_pose_batch(self, ot_system,
